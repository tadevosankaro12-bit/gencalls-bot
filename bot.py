...
def cb_routing(c):
    ...
    text = (
        ...
        "   _Номера +7 идут через Zvonok, остальные через SMS.RU._\n\n"
        f"2. 🇷🇺 **Только Zvonok (+7)** {'(АКТИВЕН ✅)' if cur == 'zvonok' else ''}\n\n"
        f"3. 🌍 **Только SMS.RU (+374 / Весь Мир)** {'(АКТИВЕН ✅)' if cur == 'smsru' else ''}"
    )
...
def on_set_route(c):
    ...
    u["routing_mode"] = mode
    save_json(DB_FILE, db)
    bot.answer_callback_query(c.id, "✅ Маршрут сохранён!")
    cb_routing(c)
...
def step_phone_input(m):
    ...
    if not phone or len(phone) < 8:
        bot.reply_to(m, "❌ Некорректный номер. Введите с кодом страны (+79991234567):")
        return
...
def cb_account(c):
    ...
    text = (
        ...
        f"⚙️ Маршрутизация: `{u.get('routing_mode', 'auto')}`\n\n"
    )
    if history:
        text += "🎙️ **Последние звонки:**\n"
        for h in history[-3:]:
...
# ================= СКРЫТАЯ АДМИН-ПАНЕЛЬ (ТОЛЬКО ПО КОМАНДЕ /admin) =================
...
def on_check_services(c):
    ...
    try:
        ...
        if "phone" in str(rj).lower() or "status" in rj:
            z_status = "✅ Подключен (API отвечает)"
        else:
...
def step_adm_uid_bal(m):
    ...
    u = get_user(uid_text)
    bot.reply_to(m, f"Юзер найден (Баланс: **{u.get('balance_rub', 0)} ₽**).\nСколько рублей начислить? (например: `49`, `100`):")
...
def step_adm_broadcast(m):
    ...
    bot.reply_to(m, f"✅ Рассылка завершена! Отправлено: {sent} пользователям.")
    show_admin_panel(m.chat.id)
