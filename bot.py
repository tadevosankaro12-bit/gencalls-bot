...
MAIN_TEXT_BANNER = (
    ...
    "Разыграйте друзей, коллег или знакомых с помощью голосовых сценариев!\n\n"
...
def cmd_start(m):
    ...
    # Реферальная система (строго 1 человек)
...
def cb_confirm_call_execute(c):
    ...
    Thread(target=simulate_call).start()
...
def cb_faq(c):
    ...
    kb.row(types.InlineKeyboardButton("🔙 Главное меню", callback_data="back_main"))
    safe_nav(c, "ℹ️ <b>GenCalls</b> — бот анонимных пранк-звонков.\nВсе вызовы производятся через виртуальные АТС.", reply_markup=kb)
